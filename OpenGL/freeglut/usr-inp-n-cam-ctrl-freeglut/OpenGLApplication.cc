...
void OpenGLApplication::Keyboard(unsigned char key, int x, int y) {
	...
	std::clog << "Distance = " << m_cameraDistance << std::endl;
}
...
void OpenGLApplication::Special(int key, int x, int y) {
	...
	std::clog << "Yaw = " << m_cameraYaw
	          << " Pitch = " << m_cameraPitch << std::endl;
...
void OpenGLApplication::Idle() {
	...
	// also draw it red
	DrawBox(Vec3f(1, 1, 1), Vec3f(1.0f, 0.2f, 0.2f));
...
void OpenGLApplication::UpdateCamera() {
	...
	// create a Vector3 to represent the camera 
	// position and scale it up if its value is too small.
	Vec3f forward(cameraPosition[0], cameraPosition[1], cameraPosition[2]);
	...
	Vec3f right = m_upVector.cross(forward);

	// create a quaternion that represents the camera's roll
	Quatf roll(right, - pitch);
	...
	// final position
	cameraPosition = Matrix3x3f(rotation) * Matrix3x3f(roll) * cameraPosition;
...
void OpenGLApplication::DrawBox(const Vec3f &halfSize, const Vec3f &color) {
	...
	Vec3f vertices[8]={	
	Vec3f(halfWidth,halfHeight,halfDepth),
	Vec3f(-halfWidth,halfHeight,halfDepth),
	Vec3f(halfWidth,-halfHeight,halfDepth),	
	Vec3f(-halfWidth,-halfHeight,halfDepth),	
	Vec3f(halfWidth,halfHeight,-halfDepth),
	...
	for (int i = 0; i < 36; i += 3) {
		...
		const Vec3f &vert1 = vertices[indices[i]];
		const Vec3f &vert2 = vertices[indices[i+1]];
		const Vec3f &vert3 = vertices[indices[i+2]];

		// create a normal that is perpendicular to the 
		// face (use the cross product)
		Vec3f normal = (vert3-vert1).cross(vert2-vert1);
		normal.normalize ();
...
void OpenGLApplication::RotateCamera(float &angle, float value) {
...
void OpenGLApplication::ZoomCamera(float distance) {
